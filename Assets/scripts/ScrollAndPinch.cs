...
class ScrollAndPinch : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.touchCount >= 1)
        {
            ...
            if(currCamPos.x > maxX){
                currCamPos.x = maxX;
            }
            if(currCamPos.x < minX){
                currCamPos.x = minX;
            }
            if(currCamPos.y > maxY){
                currCamPos.y = maxY;
            }
            if(currCamPos.y < minY){
                currCamPos.y = minY;
            }
            if(currCamPos.z > maxZ){
                currCamPos.z = maxZ;
            }
            if(currCamPos.z < minZ){
                currCamPos.z = minZ;
            }

             Camera.transform.position = currCamPos;
